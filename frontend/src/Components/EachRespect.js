...
const EachRespect = (props) => {
    ...
    const getSender = async () => {
        const res = await axios.get(`https://imagebook-digilabs.herokuapp.com/getUsers/${props.sender}`)
        const data = res?.data;
        setSender(data[0]);
    }

    const getReciever = async () => {
        const res = await axios.get(`https://imagebook-digilabs.herokuapp.com/getUsers/${props.reciever}`)
        const data = res?.data;
        setReciever(data[0]);
    }
