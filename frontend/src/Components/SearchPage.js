  ...
  return (
    <div>
      ...
      <div className="">
        ...
        {(input.length !== 0 && unreg.filter((obj) => obj.number.includes(input)).length!==0 && arr.filter((obj) => obj.number.includes(input)).length!==0) &&
        (
          <>
        ...
        {(input.length !== 0 && unreg.filter((obj) => obj.number.includes(input)).length===0 && arr.filter((obj) => obj.number.includes(input)).length===0) && <>
            <p style={{ color: "#5E849C" }} className="text-sm font-semibold ml-[20px] mt-[20px]">
                Phonebook
              </p>
              <div>
              <div style={{backgroundColor:"#F5F8FA"}} className="flex gap-2 w-11/12 mx-auto p-[12px]  rounded-xl">
             <img style={{borderRadius:"200px",width:"60px",height:"60px",marginRight:"12px"}} src="https://i.stack.imgur.com/l60Hf.png" />
            <div>
                <p className="text-lg">{input}</p>
                     <p style={{backgroundColor:"#E8E8E8"}} className="text-sm font-bold p-[4px] rounded-md">Unregistered</p>
